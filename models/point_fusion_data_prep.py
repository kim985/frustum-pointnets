...
pcd_path = r"C:\Users\faria\PycharmProjects\frustum-pointnets\dataset\KITTI\object\training\velodyne"
label_path = r"C:\Users\faria\PycharmProjects\frustum-pointnets\dataset\KITTI\object\training\label_2"
calib_path = r"C:\Users\faria\PycharmProjects\frustum-pointnets\dataset\KITTI\object\training\calib"
img_path = r"C:\Users\faria\PycharmProjects\frustum-pointnets\dataset\KITTI\object\training\image_2"
...
def process(velodyne_path, label_path=None, calib_path=None, dataformat="bin", label_type="txt", is_velo_cam=False):
    ...
    for d in filenames_velo:
        value = d[0:6]
        print(value)
        velo_path = os.path.join(velodyne_path, value + '.bin')
        cal_path = os.path.join(calib_path, value + '.txt')
        lab_path = os.path.join(label_path, value + '.txt')
        cur_points = load_pc_from_bin(velo_path)
        cur_calib = read_calib_file(cal_path)
        proj_velo = proj_to_velo(cur_calib)[:, :3]
        places, rotates, size, classes = read_labels(lab_path, label_type, cal_path, is_velo_cam=is_velo_cam,
                                                     proj_velo=proj_velo)
...
if __name__ == '__main__':
    ...
    #     # layer_names = [layer.name for layer in resnet_model.layers]
    #     # print(layer_names)
