...
class FrustumDataset(object):
    ...
    def __init__(self, npoints, split,
                 ...
                 overwritten_data_path=None, from_rgb_detection=False, one_hot=False, subset=None):
        '''
        Input:
        ...
        if from_rgb_detection:
            ...
            with open(overwritten_data_path, 'rb') as fp:
                ...
                if subset:
                    self.id_list = self.id_list[:subset]
                    self.box2d_list = self.box2d_list[:subset]
                    self.box3d_list = self.box3d_list[:subset]
                    self.input_list = self.input_list[:subset]
                    self.label_list = self.label_list[:subset]
                    self.type_list = self.type_list[:subset]
                    self.heading_list = self.heading_list[:subset]
                    # frustum_angle is clockwise angle from positive x-axis
                    self.frustum_angle_list = self.frustum_angle_list[:subset]
                    self.size_list = self.size_list[:subset]
